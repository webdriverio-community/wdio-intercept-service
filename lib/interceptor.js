...
var interceptor = {
  ...
  excludeUrls: function excludeUrls(urls, done) {
    var NAMESPACE = '__webdriverajax';
    if (window[NAMESPACE] != undefined) {
      window[NAMESPACE].excludedUrls =
        window[NAMESPACE].excludedUrls.concat(urls);
    }
    done(window[NAMESPACE]);
  },
  ...
  setup: function setup(done) {
    ...
    function pushToSessionStorage(req) {
      ...
      if (
        window[NAMESPACE].excludedUrls.find(function (url) {
          return req.url.match(new RegExp(url));
        })
      ) {
        return;
      }
