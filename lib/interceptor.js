...
var interceptor = {
  ...
  excludeUrls: function excludeUrls(urls, done) {
    const NAMESPACE = '__webdriverajax';
    if (window[NAMESPACE] != undefined) {
      window[NAMESPACE].excludedUrls = window[NAMESPACE].excludedUrls.concat(urls);
    }
    done(window[NAMESPACE]);    
  },
  ...
  setup: function setup(done) {
    ...
    window[NAMESPACE] = { interceptorDisabled: false, excludedUrls: [], requests: [] };
    ...
    function pushToSessionStorage(req) {
      ...
      if(window[NAMESPACE].excludedUrls.find(url => req.url.match(new RegExp(url)))){
        return;
      }
