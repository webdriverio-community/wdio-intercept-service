...
var interceptor = {
  setup: function setup(done) {
    ...
    function replaceXHR() {
      ...
      window.XMLHttpRequest = function() {
        ...
        xhr.open = function() {
          arguments[1] = redirect(arguments[1]);
    ...
    function redirect(input) {
      var redirects = window.__webdriverredirects;
      for(var i = 0; i < redirects.length; i++) {
        var pattern = new RegExp(redirects[i].patt);
        if (pattern.test(input)) {
          return input.replace(pattern, redirects[i].newUrl);
        }
        return input;
      }
    }
  ...
  setRedirections: function(redirectList, done) {
    window.__webdriverredirects = redirectList;
    done();
  },

  resetRedirections: function(done) {
    window.__webdriverredirects = [];
    done();
  }
};
